...
#include "spirt/spirt_client_runner.hpp"
...
int main(int argc, char** argv) {
    ...
    sse::logger::set_benchmark_file("benchmark_spirt_client.out");
    
    sse::crypto::init_crypto_lib();
     std::string search_arry[20];
...
//             break;
    ...
    std::unique_ptr<sse::spirt::SpirtClientRunner> client_runner;
    ...
    client_runner.reset( new sse::spirt::SpirtClientRunner("localhost:4240", client_db, setup_size, n_keywords) );
...
//        auto post_callback = [&writer, &res_size, &writer_lock](index_type i)
        ...
        auto gen_callback = [&client_runner](const std::string &s, const std::string & i)
        {
           //std::cout<<"gen_callback"<<"keyword"<<s<<std::endl<<"index"<<sse::spirt::m_bytetobit(i)<<std::endl;
        

            // sse::logger::log(sse::logger::INFO) << "i: " << sizeof(i) <<", index: "<< index.size() << std::endl;
            
            client_runner->update(s, i);
        };
    ...
    for (std::string &kw : keywords) {
        ...
        std::vector<std::string> v = sse::spirt::split(kw, "to"); //可按多个字符来分隔;
 ...
 // std::cout << std::endl;
  ...
  right=static_cast<int>(strtol(v[1].c_str(),&end,10));
      //std::cout<<"?"<<left<<"?"<<right;
      int count=0;
      search_count=sse::spirt::TRsearch(search_arry,0,m-1,left,right,count);
