...
#include "spirt_client_runner.hpp"

#include "spirt_net_types.hpp"
...
SpirtClientRunner::SpirtClientRunner(const std::string& address, const std::string& path, size_t setup_size, uint32_t n_keywords)
    : bulk_update_state_{0}, update_launched_count_(0), update_completed_count_(0)
{
    ...
    stub_ = spirt::Spirt::NewStub(channel);
                    
    if (is_directory(path)) {
        // try to initialize everything from this directory

        client_ = SpirtClient::construct_from_directory(path);
        ...
        client_ = SpirtClient::init_in_directory(path, n_keywords);
    ...
    update_completion_thread_ = new std::thread(&SpirtClientRunner::update_completion_loop, this);
}


SpirtClientRunner::~SpirtClientRunner()
{
    update_cq_.Shutdown();
...
bool SpirtClientRunner::send_setup(const size_t setup_size) const
{
    grpc::ClientContext context;
    spirt::SetupMessage message;
...
const SpirtClient& SpirtClientRunner::client() const
{
    if (!client_) {
...
std::list<index_type> SpirtClientRunner::search(const std::string& keyword, std::function<void(index_type)> receive_callback) const
{
    logger::log(logger::TRACE) << "Search " << keyword << std::endl;
    
    grpc::ClientContext context;
    spirt::SearchRequestMessage message;
    spirt::SearchReply reply;
    
    message = request_to_message(client_->search_request(keyword));
    
    std::unique_ptr<grpc::ClientReader<spirt::SearchReply> > reader( stub_->search(&context, message) );
...
std::list<index_type> SpirtClientRunner::Rsearch( std::string * keyword, int count, std::function<void(index_type)> receive_callback) const
{
    logger::log(logger::TRACE) << "Search " << keyword << std::endl;
...
void SpirtClientRunner::update(const std::string& keyword, index_type index)
{
    grpc::ClientContext context;
    spirt::UpdateRequestMessage message;
    google::protobuf::Empty e;
...
void SpirtClientRunner::async_update(const std::string& keyword, index_type index)
{
    grpc::ClientContext context;
    spirt::UpdateRequestMessage message;
    ...
    void SpirtClientRunner::update_in_session(const std::string& keyword, index_type index)
    {
        spirt::UpdateRequestMessage message = request_to_message(client_->update_request(keyword, index));
...
void SpirtClientRunner::wait_updates_completion()
{
...
void SpirtClientRunner::start_update_session()
{
...
void SpirtClientRunner::end_update_session()
{
    if (!bulk_update_state_.writer) {
...
void SpirtClientRunner::update_completion_loop()
{
    update_tag_type* tag;
    bool ok = false;
...
std::ostream& SpirtClientRunner::print_stats(std::ostream& out) const
{
...
SearchRequestMessage request_to_message(const SearchRequest& req)
{
    ...
    mes.set_key(req.key.data(),req.key.size());
