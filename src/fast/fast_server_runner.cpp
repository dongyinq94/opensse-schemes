...
#include "fast_server_runner.hpp"
...
namespace sse {
    namespace fast {
        ...
        const std::string FastImpl::pairs_map_file = "pairs.dat";

FastImpl::FastImpl(const std::string& path) :
storage_path_(path), async_search_(true)
{
    if (is_directory(storage_path_)) {
        ...
        server_.reset( new FastServer(pairs_map_path) );
...
grpc::Status FastImpl::setup(grpc::ServerContext* context,
                    const fast::SetupMessage* message,
                    google::protobuf::Empty* e)
{
    ...
    try {
        ...
        server_.reset(new FastServer(pairs_map_path, message->setup_size()));
...
grpc::Status FastImpl::search(grpc::ServerContext* context,
                                const fast::SearchRequestMessage* mes,
...
grpc::Status FastImpl::sync_search(grpc::ServerContext* context,
                                     const fast::SearchRequestMessage* mes,
                                     grpc::ServerWriter<fast::SearchReply>* writer)
{
    if (!server_) {
...
grpc::Status FastImpl::async_search(grpc::ServerContext* context,
                                      const fast::SearchRequestMessage* mes,
                                      grpc::ServerWriter<fast::SearchReply>* writer)
{
    if (!server_) {
...
grpc::Status FastImpl::update(grpc::ServerContext* context,
                    const fast::UpdateRequestMessage* mes,
                    google::protobuf::Empty* e)
{
    ...
    return grpc::Status::OK;
        ...
        grpc::Status FastImpl::bulk_update(grpc::ServerContext* context,
                                        grpc::ServerReader<fast::UpdateRequestMessage>* reader, google::protobuf::Empty* e)
        {
            if (!server_) {
                // problem, the server is already set up
                return grpc::Status(grpc::FAILED_PRECONDITION, "The server is not set up");
...
std::ostream& FastImpl::print_stats(std::ostream& out) const
{
    if (server_) {
...
bool FastImpl::search_asynchronously() const
{
    return async_search_;
}

void FastImpl::set_search_asynchronously(bool flag)
{
    async_search_ = flag;
}
...
void run_fast_server(const std::string &address, const std::string& server_db_path, grpc::Server **server_ptr, bool async_search) {
    ...
    FastImpl service(server_db_path);
