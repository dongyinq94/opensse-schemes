...
#include "fastio_client_runner.hpp"

#include "fastio_net_types.hpp"
...
FastioClientRunner::FastioClientRunner(const std::string& address, const std::string& path, size_t setup_size, uint32_t n_keywords)
    : bulk_update_state_{0}, update_launched_count_(0), update_completed_count_(0)
{
    ...
    stub_ = fastio::Fastio::NewStub(channel);
                    
    if (is_directory(path)) {
        // try to initialize everything from this directory

        client_ = FastioClient::construct_from_directory(path);
        ...
        client_ = FastioClient::init_in_directory(path, n_keywords);
    ...
    update_completion_thread_ = new std::thread(&FastioClientRunner::update_completion_loop, this);
}


FastioClientRunner::~FastioClientRunner()
{
    update_cq_.Shutdown();
...
bool FastioClientRunner::send_setup(const size_t setup_size) const
{
    grpc::ClientContext context;
    fastio::SetupMessage message;
...
const FastioClient& FastioClientRunner::client() const
{
    if (!client_) {
...
std::list<index_type> FastioClientRunner::search(const std::string& keyword, std::function<void(index_type)> receive_callback) const
{
    logger::log(logger::TRACE) << "Search " << keyword << std::endl;
    
    grpc::ClientContext context;
    fastio::SearchRequestMessage message;
    fastio::SearchReply reply;
    
    message = request_to_message(client_->search_request(keyword));
    
    std::unique_ptr<grpc::ClientReader<fastio::SearchReply> > reader( stub_->search(&context, message) );
...
void FastioClientRunner::update(const std::string& keyword, index_type index)
{
    grpc::ClientContext context;
    fastio::UpdateRequestMessage message;
    google::protobuf::Empty e;
...
void FastioClientRunner::async_update(const std::string& keyword, index_type index)
{
    grpc::ClientContext context;
    fastio::UpdateRequestMessage message;
    ...
    void FastioClientRunner::update_in_session(const std::string& keyword, index_type index)
    {
        fastio::UpdateRequestMessage message = request_to_message(client_->update_request(keyword, index));
...
void FastioClientRunner::wait_updates_completion()
{
...
void FastioClientRunner::start_update_session()
{
...
void FastioClientRunner::end_update_session()
{
    if (!bulk_update_state_.writer) {
...
void FastioClientRunner::update_completion_loop()
{
    update_tag_type* tag;
    bool ok = false;
...
// bool FastioClientRunner::load_inverted_index(const std::string& path)
// {
//     try {
...
std::ostream& FastioClientRunner::print_stats(std::ostream& out) const
{
