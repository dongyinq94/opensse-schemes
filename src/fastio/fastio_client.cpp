...
namespace sse {
    namespace fastio {
        ...
        const std::string FastioClient::counter_map_file__ = "counters.dat";
        ...
        { // 如果存在该目录，密钥文件已经存在，执行初始化
            ...
            return std::unique_ptr<FastioClient>(new FastioClient(counter_map_path, master_key_buf.str(), state_key_buf.str() ));
        }
        ...
        { // 在文件夹下执行初始化，文件夹中没有密钥文件
            ...
            auto c_ptr =  std::unique_ptr<FastioClient>(new FastioClient(counter_map_path, n_keywords));
            c_ptr->write_keys(dir_path); // keys are chose randomly from class PRFBase !
            
            return c_ptr;
        }
        
        FastioClient::FastioClient(const std::string& token_map_path, const size_t tm_setup_size) :
        k_prf_(), s_prf_(), counter_map_(token_map_path)
        {
            
        }

        FastioClient::FastioClient(const std::string& token_map_path, const std::string& derivation_master_key, const std::string& state_master_key) :
        ...
        SearchRequest FastioClient::search_request(const std::string &keyword) const
        {
            // TODO need to be done according to FAST's specification
            uint32_t kw_counter;
            bool found;
            SearchRequest req;
            req.add_count = 0;
